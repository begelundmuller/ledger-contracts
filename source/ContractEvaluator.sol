...
contract ContractEvaluator is ContractChecker {
  ...
  function handleObservation(uint key, bytes8 name, bytes8 label, uint time) internal returns (int);
  ...
  function evaluateContract(uint key, uint contractId, int scale)
  internal returns (uint) {
    ...
    if (c.variant == ContrVariant.Empty) {
      ...
      if (consts[c.const1].integer <= int256(block.timestamp)) {
        return evaluateContract(key, c.contr1, scale);
      }
    } else if (c.variant == ContrVariant.And) {
      return evaluateAndContract(key, contractId, scale);
    } else if (c.variant == ContrVariant.Scale) {
      return evaluateScaleContract(key, contractId, scale);
      ...
      return evaluateIfWithinContract(key, contractId, scale);
    }
  ...
  function evaluateAndContract(uint key, uint contractId, int scale)
  internal returns (uint) {
    // Get contract
    Contr c = contrs[contractId];

    // Reduce both contrs
    uint outContractId1 = evaluateContract(key, c.contr1, scale);
    uint outContractId2 = evaluateContract(key, c.contr2, scale);
  ...
  function evaluateScaleContract(uint key, uint contractId, int scale)
  internal returns (uint) {
    // Get contract
    Contr c = contrs[contractId];

    // Evaluate expression for scale value
    uint outExprId = evaluateExpression(key, c.expr1);
    ...
    uint outContractId = evaluateContract(key, c.contr1, k.integer * scale);
  ...
  function evaluateIfWithinContract(uint key, uint contractId, int scale)
  internal returns (uint) {
    // Get contract
    Contr c = contrs[contractId];
    ...
    for (uint i = 0; i < uint(n); i++) {
      ...
      if (uint(t) <= block.timestamp) {
        ...
        uint exprId = evaluateExpression(key, c.expr1);
        popEnv();
        Expr expr = exprs[exprId];
        if (expr.variant == ExprVariant.Constant) {
          Const k = consts[expr.const1];
          if (k.variant == ConstVariant.Boolean && k.boolean) {
            result = true;
          }
        }
      }
    ...
    if (result) {
      ...
      return evaluateContract(key, c.contr2, scale);
    }
  }

  /// Evaluates expression and returns the ID of the reduced expression
  function evaluateExpression(uint key, uint expressionId)
  internal returns (uint) {
    // Get expression
  ...
  function evaluateExpressionObservation(uint key, uint expressionId)
  internal returns (uint) {
    // Get expression and evaluate sub-expressions
    Expr e = exprs[expressionId];
    Expr e1 = exprs[evaluateExpression(key, e.expr1)];
    Expr e2 = exprs[evaluateExpression(key, e.expr2)];
    ...
    // Get value from feed
    int k = handleObservation(key, e.identifier1, k1.label, uint(k2.integer));
    if (k == 0) {
      return expressionId;
    } else {
  ...
  function evaluateExpressionAcc(uint key, uint expressionId)
  internal returns (uint) {
    // Get
    Expr e = exprs[expressionId];
    ...
    for (uint i = 0; i <= uint(n); i++) {
      ...
      currentExpr = evaluateExpression(key, e.expr1);
      popEnv();
      popEnv();
    }
  ...
  function evaluateExpressionUnaryOp(uint key, uint expressionId)
  internal returns (uint) {
    // Get expr in question
    Expr e = exprs[expressionId];
    uint outExpressionId = evaluateExpression(key, e.expr1);
  ...
  function evaluateExpressionBinaryOp(uint key, uint expressionId)
  internal returns (uint) {
    // Get expr in question
    Expr e = exprs[expressionId];

    // Evaluate each sub expr
    uint exprId1 = evaluateExpression(key, e.expr1);
    uint exprId2 = evaluateExpression(key, e.expr2);
